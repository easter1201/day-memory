...
- [x] D-Day 계산 로직 구현
- [ ] 반복 이벤트 자동 생성 로직 (is_recurring)
- [ ] 이벤트 타입별 필터링 API (GET /api/events?type={type})
...
#### 소셜 로그인
- [ ] OAuth2 클라이언트 설정 (Spring Security)
- [ ] Google OAuth 연동 (GET /api/auth/google)
  - [ ] Google API Console 설정
  - [ ] Redirect URI 설정
- [ ] Kakao OAuth 연동 (GET /api/auth/kakao)
  - [ ] Kakao Developers 설정
  - [ ] Redirect URI 설정
- [ ] 소셜 로그인 콜백 처리
- [ ] JWT 토큰 발급 (소셜 로그인 성공 시)
...
#### Logback 설정
- [ ] Logback 설정 파일 작성 (logback-spring.xml)
- [ ] 로그 레벨 설정 (INFO: 운영, DEBUG: 개발)
- [ ] 로그 파일 로테이션 (일별)
- [ ] 요청/응답 로깅 인터셉터
- [ ] 에러 로그 분류 (ERROR: 5xx, WARN: 4xx)
