...
- [ ] Sidebar 컴포넌트 구현
  ...
  - [ ] 반응형 대응 (모바일 토글)
- [ ] Footer 컴포넌트 구현
- [ ] PageLayout 컴포넌트 구현 (Header + Sidebar + Content 조합)
- [ ] AuthLayout 컴포넌트 구현 (인증 페이지용)
...
- [ ] 회원가입 폼 구현
  ...
  - [ ] 비밀번호 확인 필드
  - [ ] 닉네임 입력 필드
  - [ ] 유효성 검증 (이메일 형식, 비밀번호 강도, 일치 여부)
  - [ ] 회원가입 버튼
- [ ] API 연동 (authApi.signup)
- [ ] 가입 성공 시 /login 리다이렉트
- [ ] 에러 처리 (이미 존재하는 이메일 등)
...
- [ ] WelcomeBanner 컴포넌트
  - [ ] 사용자 이름 표시
  - [ ] 인사 메시지
- [ ] UpcomingEventsWidget 컴포넌트
  - [ ] 향후 30일 이내 이벤트 5개 표시
  - [ ] D-Day 계산 및 표시
  - [ ] 이벤트 클릭 시 상세 페이지 이동
- [ ] StatisticsCards 컴포넌트
  - [ ] 총 이벤트 수
  - [ ] 총 선물 수
  - [ ] 이번 달 이벤트 수
- [ ] QuickActionButtons 컴포넌트
  - [ ] 새 이벤트 추가 버튼
  - [ ] 새 선물 추가 버튼
  - [ ] AI 추천 요청 버튼
- [ ] API 연동 (dashboardApi)
